...
public class Rankings extends Thread{
	static final Logger log = Logger.getLogger( Rankings.class.getName() );
	
	static DBConnect dbc =new DBConnect("root",null,null);
	
	public static volatile String[][] top10nomoi = new String[10][2];
	public static volatile int[] topNomoi;
	...
	public static volatile String[][] top100perioxes = new String[100][2];
	public static volatile int[] topPerioxes;
	...
	public static volatile String top100PerioxesFormatted = "";
	public static volatile String top10NomoiFormatted = "";
	public static volatile String top100Formatted = "";
	
	
	public static volatile String[][] top100;
	public static volatile String[][][][] areaTop100;
	public static volatile String[][][] nomosTop100;
	
	
	public void run(){
		while(!isInterrupted()){
			//write previous day's results to db
			if(LiveRankings.stopwatch!=666){
				if(System.currentTimeMillis()-LiveRankings.stopwatch < 240000){
				...
				}else if(System.currentTimeMillis()-LiveRankings.stopwatch >= 540000){
					try{
						sleep(65000);	//sleep because LiveRankings is about to trigger, need to avoid conflict
					}catch(Exception e){
				...
				//
				//clear previous day's live rankings
				LiveRankings.nomoiTemp = Arrays.copyOf(nomoi, nomoi.length);
				LiveRankings.perioxesTemp = Arrays.copyOf(perioxes, perioxes.length);
				LiveRankings.activeTodayNomoi = null;
				LiveRankings.activeTodayNomoi = new int[nomoi.length];
				LiveRankings.activeTodayPerioxes = null;
				LiveRankings.activeTodayPerioxes = new int[perioxes.length];
				LiveRankings.gamesTodayNomoi = null;
				LiveRankings.gamesTodayNomoi = new int[nomoi.length];
				LiveRankings.gamesTodayPerioxes = null;
				LiveRankings.gamesTodayPerioxes = new int[perioxes.length];
				LiveRankings.nikesTodayNomoi = null;
				LiveRankings.nikesTodayNomoi = new int[nomoi.length];
				LiveRankings.nikesTodayPerioxes = null;
				LiveRankings.nikesTodayPerioxes = new int[perioxes.length];
				LiveRankings.ittesTodayNomoi = null;
				LiveRankings.ittesTodayNomoi = new int[nomoi.length];
				LiveRankings.ittesTodayPerioxes = null;
				LiveRankings.ittesTodayPerioxes = new int[perioxes.length];
				LiveRankings.revengeTodayNomoi = null;
				LiveRankings.revengeTodayNomoi = new double[nomoi.length];
				LiveRankings.revengeTodayPerioxes = null;
				LiveRankings.revengeTodayPerioxes = new double[perioxes.length];
				//
			}
			System.out.println("Loading top 100...");
			top100=dbc.getTop100();
			top100Formatted="";
			for(int i=0;i<100;i++){
				if(i!=99){
					top100Formatted+=top100[i][0]+"-$-"+top100[i][1]+"-$-"+top100[i][2]+"-*-";
				}else{
					top100Formatted+=top100[i][0]+"-$-"+top100[i][1]+"-$-"+top100[i][2];
				}
			}
			System.out.println("Loading top nomoi...");
			nomosTop100 = dbc.getNomosTop100();
			topNomoi = dbc.getTopNomoi();
			nomoiTemp = Arrays.copyOf(nomoi, nomoi.length);
			bubbleSort(topNomoi,nomoiTemp);
			for(int i =0;i<10;i++){
				top10nomoi[i][0] = nomoiTemp[i];
				top10nomoi[i][1] = String.valueOf(topNomoi[i]);
			}	
			System.out.println("Top nomoi loaded!");
			
			
			System.out.println("Loading top perioxes...");
			areaTop100=dbc.getAreaTop100();
			topPerioxes = dbc.getTopPerioxes();
			perioxesTemp = Arrays.copyOf(perioxes, perioxes.length);
			bubbleSort(topPerioxes,perioxesTemp);
			top100perioxes = new String[100][2];
			for(int i =0;i<100;i++){
				top100perioxes[i][0] = perioxesTemp[i]; //name
				top100perioxes[i][1] = String.valueOf(topPerioxes[i]); //points
			}
			
			System.out.println("Top perioxes loaded!");
			
			top100PerioxesFormatted = "";
			for(int i =0;i<100;i++){
				if(i!=99){
					top100PerioxesFormatted+=top100perioxes[i][0]+"-$-"+top100perioxes[i][1]+"-*-";
				}else{
					top100PerioxesFormatted+=top100perioxes[i][0]+"-$-"+top100perioxes[i][1];
				}
			}
			...
			for(int i =0;i<10;i++){
				if(i!=9){
					top10NomoiFormatted+=top10nomoi[i][0]+"-$-"+top10nomoi[i][1]+"-*-";
				}else{
					top10NomoiFormatted+=top10nomoi[i][0]+"-$-"+top10nomoi[i][1];
				}
			}
	...
	public void bubbleSort(int[] arr,String[] nomoi) {
