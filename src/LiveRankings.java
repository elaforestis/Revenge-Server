...
public class LiveRankings extends Thread{
	static final Logger log = Logger.getLogger( LiveRankings.class.getName() );
	
	static DBConnect dbc =new DBConnect("root",null,null);
	public static final int INTERVAL = 20000; //300.000
	

	public static volatile String[][] top100Daily;
	

	public static volatile String[][][][] areaTop100Daily;
	

	public static volatile String[][][] nomosTop100Daily;
	...
	/////////////////////
	public static volatile int[] activeTodayNomoi = new int[51];
	public static volatile int[] activeTodayPerioxes = new int[1146];
	
	public static volatile int[] gamesTodayNomoi = new int[51];
	public static volatile int[] gamesTodayPerioxes = new int[1146];
	
	public static volatile int[] nikesTodayNomoi = new int[51];
	public static volatile int[] nikesTodayPerioxes = new int[1146];
	
	public static volatile int[] ittesTodayNomoi = new int[51];
	public static volatile int[] ittesTodayPerioxes = new int[1146];
	
	public static volatile double[] revengeTodayNomoi = new double[51];
	public static volatile double[] revengeTodayPerioxes = new double[1146];
	//////////////////////

	public static volatile String top100DailyFormatted ="";
	public static volatile String top10NomoiTodayFormatted = "";
	public static volatile String top100PerioxesTodayFormatted="";

	public static volatile String[] nomoiTemp;
	public static volatile String[] perioxesTemp;
	
	public static volatile long stopwatch = 666;
	...
	public void run(){
		...
		while(!isInterrupted()){
			...
			top100Daily = dbc.getTop100Daily();
			areaTop100Daily = dbc.getAreaTop100Daily();
			nomosTop100Daily = dbc.getNomosTop100Daily();
			
			nomoiTemp = null;
			nomoiTemp = Arrays.copyOf(nomoi,nomoi.length);
			perioxesTemp = null;
			perioxesTemp = Arrays.copyOf(perioxes, perioxes.length);
			revengeTodayNomoi = null;
			revengeTodayNomoi = new double[51];
			revengeTodayPerioxes = null;
			revengeTodayPerioxes = new double[1146];
			//System.out.println(Arrays.deepToString(areaTop10[4][111])); //attiki->nea smirni
			//////////////calculate revenge
			//nomoi
			for(int i =0;i<nomoi.length;i++){
				if(gamesTodayNomoi[i]!=0){
					if(gamesTodayNomoi[i]<=100){
						revengeTodayNomoi[i] = (nikesTodayNomoi[i]-ittesTodayNomoi[i])/2.0+50;
					}else{
						revengeTodayNomoi[i] = (nikesTodayNomoi[i]/(double)gamesTodayNomoi[i])*100;
						if(activeTodayNomoi[i]>100 && Double.compare(gamesTodayNomoi[i]/(double)activeTodayNomoi[i],5.0)>0){
							if(Double.compare(gamesTodayNomoi[i]/(double)activeTodayNomoi[i],46.0)<0){
								revengeTodayNomoi[i]+=(1/25.0)*(9*(gamesTodayNomoi[i]/(double)activeTodayNomoi[i])-40);
							}else{
								revengeTodayNomoi[i]+=(1/25.0)*(9*46-40);
							}
						}else if(activeTodayNomoi[i]<=100 && Double.compare(gamesTodayNomoi[i]/(double)activeTodayNomoi[i],5.0)>0){
							if(Double.compare(gamesTodayNomoi[i]/(double)activeTodayNomoi[i],46.0)<0){
								revengeTodayNomoi[i]+=((1/25.0)*(9*(gamesTodayNomoi[i]/(double)activeTodayNomoi[i])-40))*(activeTodayNomoi[i]/100.0);
							}else{
								revengeTodayNomoi[i]+=((1/25.0)*(9*46-40))*(activeTodayNomoi[i]/100.0);
							}
						}
					}
				}
			...
			for(int i =0;i<perioxes.length;i++){
				if(gamesTodayPerioxes[i]!=0){
					if(gamesTodayPerioxes[i]<=100){
						revengeTodayPerioxes[i] = (nikesTodayPerioxes[i]-ittesTodayPerioxes[i])/2.0+50;
					}else{
						revengeTodayPerioxes[i] = (nikesTodayPerioxes[i]/(double)gamesTodayPerioxes[i])*100;
						if(activeTodayPerioxes[i]>100 && Double.compare(gamesTodayPerioxes[i]/(double)activeTodayPerioxes[i],5.0)>0){
							if(Double.compare(gamesTodayPerioxes[i]/(double)activeTodayPerioxes[i],46.0)<0){
								revengeTodayPerioxes[i]+=(1/25.0)*(9*(gamesTodayPerioxes[i]/(double)activeTodayPerioxes[i])-40);
							}else{
								revengeTodayPerioxes[i]+=(1/25.0)*(9*46-40);
							}
						}else if(activeTodayPerioxes[i]<=100 && Double.compare(gamesTodayPerioxes[i]/(double)activeTodayPerioxes[i],5.0)>0){
							if(Double.compare(gamesTodayPerioxes[i]/(double)activeTodayPerioxes[i],46.0)<0){
								revengeTodayPerioxes[i]+=((1/25.0)*(9*(gamesTodayPerioxes[i]/(double)activeTodayPerioxes[i])-40))*(activeTodayPerioxes[i]/100.0);
							}else{
								revengeTodayPerioxes[i]+=((1/25.0)*(9*46-40))*(activeTodayPerioxes[i]/100.0);
							}
						}
					}
				}
			...
			//
			////////////
			bubbleSort(revengeTodayNomoi,nomoiTemp);
			bubbleSort(revengeTodayPerioxes,perioxesTemp);
			top10NomoiTodayFormatted="";
			for(int i =0;i<10;i++){
				if(i!=9){
					top10NomoiTodayFormatted+=nomoiTemp[i]+"-$-"+revengeTodayNomoi[i]+"-*-";
				}else{
					top10NomoiTodayFormatted+=nomoiTemp[i]+"-$-"+revengeTodayNomoi[i];
				}
			}
			top100PerioxesTodayFormatted="";
			for(int i =0;i<100;i++){
				if(i!=99){
					top100PerioxesTodayFormatted+=perioxesTemp[i]+"-$-"+revengeTodayPerioxes[i]+"-*-";
				}else{
					top100PerioxesTodayFormatted+=perioxesTemp[i]+"-$-"+revengeTodayPerioxes[i];
				}
			}
			top100DailyFormatted="";
			for(int i=0;i<100;i++){
				if(i!=99){
					top100DailyFormatted+=top100Daily[i][0]+"-$-"+top100Daily[i][1]+"-$-"+top100Daily[i][2]+"-*-";
				}else{
					top100DailyFormatted+=top100Daily[i][0]+"-$-"+top100Daily[i][1]+"-$-"+top100Daily[i][2];
				}
			}
	...
	public void bubbleSort(double[] arr,String[] n) {
